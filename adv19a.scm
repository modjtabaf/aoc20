...
(begin
(define rules (list
(cons 0 (list (list  4 1 5)))
(cons 1 (list (list  2 3 ) #\| (list  3 2)))
(cons 2 (list (list  4 4 ) #\| (list  5 5)))
(cons 3 (list (list  4 5 ) #\| (list  5 4)))
(cons 4 #\a)
(cons 5 #\b)
))
...
(define (eval-rules ids lst) (let
  (
    (ret (cons #t lst))
  )

  ;(format #t "eval-rules: ids=~a, str=~s\n" ids (list->string lst))
  (for-each
    (lambda (id)
      (if (car ret)
        (set! ret (eval-rule id (cdr ret)))
      )
    )
    ids
  )
  
  (if (car ret) ret (cons #f lst))
))

(define (eval-rule id lst) (let
  (
    (rule (cdr (assoc id rules)))
    (ret (cons #f '()))
  )

  ;(format #t "eval-rule: id=~a, rule=~a, str=~s\n" id rule (list->string lst))
  (if (not (null? lst))
    (if (pair? rule) (begin
      ; then
      (if (= (length rule) 3) (begin
        ; an or operation
        (set! ret (eval-rules (car rule) lst))
        (if (not (car ret))
          (set! ret (eval-rules (caddr rule) lst))
        )
      ))
      (if (= (length rule) 1) (begin
        (set! ret (eval-rules (car rule) lst))
      ))
    )
      ; else
      (set! ret (cons (equal? (car lst) rule) (cdr lst)))
    )
  )
...
(for-each
  (lambda (str) (let
    (
      (ret (eval-rule 0 (string->list str)))
    )
    (if (and (car ret) (null? (cdr ret))) (set! total (1+ total)))
  ))
  input
)
