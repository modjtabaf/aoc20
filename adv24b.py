...
for day in range(100):
  ...
  # find the black tiles which should be flipped
  for i, black in enumerate(blacks):
    n = 0
    for v in cmds.values():
      foo = [black[0] + v[0], black[1] + v[1]]
      if foo in blacks:
        n += 1
  ...
  # find the white tiles which should be flipped
  for i, black in enumerate(blacks):
    for v in cmds.values():
      white = [black[0] + v[0], black[1] + v[1]]
      if white in blacks or white in goblack:
        continue
      
      # found a new white tile
      n = 0
      for u in cmds.values():
        foo = [white[0] + u[0], white[1] + u[1]]
        if foo in blacks:
          n += 1
          if n > 2:
            break
      ...
      if n == 2:
        goblack.append(white)
      
  # remove the flipped black tiles
  for i, k in enumerate(gowhite):
    blacks.pop(k - i)
      
  # add the flipped white tiles
  for tile in goblack:
    blacks.append(tile)

  print("Day %d: %d" % (day+1, len(blacks)))
