...
shape = (init_state.shape[0] + 2*ncycles, init_state.shape[1] + 2*ncycles, 1 + 2*ncycles)

active_mask = np.zeros(shape, dtype=int)
active_mask[ncycles:ncycles+init_state.shape[0], ncycles:ncycles+init_state.shape[1], ncycles] = init_state
  ...
  for k in [0, 1, 2]:
    ...
    for j in [0, 1, 2]:
      ...
      for i in [0, 1, 2]:
        si1 = slices[i]
        si2 = slices[2-i]
        buffer[si1, sj1, sk1] += active_mask[si2, sj2, sk2]
